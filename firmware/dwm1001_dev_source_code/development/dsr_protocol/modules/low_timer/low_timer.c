 ...
 * @param p_context General purpose pointer that will be available at the callback handler.
 */
void lowTimerStart(app_timer_id_t timerId, uint32_t timeout, void* p_context)
{
  ret_code_t errCode = app_timer_start(timerId, usToTicks(timeout), p_context);
  APP_ERROR_CHECK(errCode);
}
